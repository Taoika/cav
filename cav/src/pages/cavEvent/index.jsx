...
export default function CavEvent() {
	...
	//最左边车辆的坐标
	const [Leftest, setLeftest] = useState(230)
	//屏幕坐标
	...
	useEffect(() => {
		if (Ncar.length > 0 && Nlane.length > 0 && Nlane[0] === 0 && Nlane[0] === 0) {
			...
			let max = { x: 100, y: 100 }
			let Lane = document.querySelector('.cavEvent-right-main');
			...
			let data = [];
			let url = 'https://qgailab.com/cav-api/3/easy-task';
			setMove(false)
			setCoor([{ x: 0, y: 0 }])
			setScrCoor([{ x: 0, y: 0 }])
			// setLoading(true)
			axiosJSONPost(url, data).
				then(response => {
					...
					response.data.data.x_list.map((v,i) => {
						data.push({list:v[0].map((value, index) => ({ x: v[0][index], y: v[1][index]*25 })) })
					})
					response.data.data.xL_list.map((v,i) => {
						data[i].list.push({ x: v[0][0], y: v[1][0]*25});
						data[i].list.push({ x: v[0][1], y: v[1][1]*25});
					})
					console.log(data);
					setCoor(data);
					setMove(true)
		...
		<div className="cavEvent">
			<Context.Provider value={{ initPosition, setInitPosition }}>
				...
				<div className="cavEvent-right">
					<div className="cavEvent-right-head">
						<Button><Space>Del <DeleteFilled /></Space></Button>
					</div>
					{refresh ? <div className='cavEvent-right-main'>
						{Ncar.map((value, index) => (
							<Car 
								...
								move={move} 
								setMove={setMove} 
								scrCoor={scrCoor.length > 10 && move ? scrCoor.map((v, i) => ({ y: v.list[index].y, x: v.list[index].x === Math.min(...v.list.map((vv, ii) => (vv.x))) ? Leftest : Leftest + (v.list[index].x - Math.min(...v.list.map((vv, ii) => (vv.x)))) })) : ''} angle={angle} key={index} setDis={setDis} setSpeed={setSpeed} 
							/>))}
